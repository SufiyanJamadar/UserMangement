    ...
    public class UserService : IUserService
    {
        ...
        public async Task<APIResponse> ConfirmRegister(int userid, string username, string otptext)
        {
            ...
            else
            {
                ...
                {
                    response.Result = "fail";
                    response.Message= "User data not found or already confirmed.";
                    return response;
                }
        ...
        public async Task<APIResponse> UserRegistration(UserRegister userRegister)
        {
            ...
            try
            {
                ...
                // duplicate Email 
                var _email = await this.context.TblUsers.Where(item => item.Email == userRegister.Email).ToListAsync();
                if (_email.Count > 0)
                {
                    isvalid = false;
                    response.Result = "fail";
                ...
                if (userRegister != null && isvalid)
                {
                    ...
                    // inside tbl_tempuser table the record is inserting but the OTP is not able to send to the receipnet
        ...
        public async Task<APIResponse> ResetPassword(string username, string oldpassword, string newpassword)
        {
            ...
            if (_user != null)
            {
                var _pwdhistory = await Validatepwdhistory(username, newpassword);
                 ...
                 {
                    response.Result = "fail";
                    response.Message = "Dont Use the same Password that used for last 3 transactions";
                }
                else
                {
        ...
        {
            APIResponse response = new APIResponse();
            var _user=await this.context.TblUsers.FirstOrDefaultAsync(item=>item.Username== username && item.Islocked==true);
            if (_user != null)
            {
                string otptext = Generaterandomnumber();
                await UpdateOtp(username, otptext,"forgetpassword");
                SendOtpMail(_user.Email, otptext, _user.Name);
                response.Result = "Pass";
        ...
        {
            bool response= false;
            var _data = await this.context.TblOtpManagers.FirstOrDefaultAsync(item => item.Username== username && item.Otptext==OTPText
        ...
        private string GenerateEmailBody(string name,string otptext)
        {
            ...
            emailbody = "<div style='width:100%; backgroud-color:grey'>";
            emailbody += "<h1>Hi " + name + " , Thanks for Registering</h1> ";
            emailbody += "<h2> Please Enter OTP text and completing registeration</h2>";
        ...
        private async Task<bool> Validatepwdhistory(string Username, string password)
        {
            ...
            if (_pwd.Count > 0)
            {
                var valid = _pwd.Where(o => o.Password == password);
                if(valid.Any())
                {
                    response = true;
                }
